...
>
> **Level adjustment:** Type `# ` at the start of a heading to increase its level (H1 → H2 → … → H6). Press Backspace at the start to decrease the level (H2 → H1 → paragraph).
...
- **Bold** — wrap text with `**double asterisks**`, or type `*text*` then press `*` again to upgrade italic to bold. Press Backspace at the left edge to downgrade: **bold** → *italic* → plain
- *Italic* — wrap text with `*single asterisks*` or `_single underscores_`. Press Backspace at the left edge to remove
...
- [Privacy Policy](privacy-policy.md) — try clicking this link!
...
**Tip:** Press `Tab` to navigate between table cells, `Shift+Tab` to go back. Drag column borders to resize columns.
...
- Read-only files cannot be renamed
...
- **Formatting preservation** — LiveMark strips inline marks before sending text to the LLM, then uses diff-based alignment to re-apply **bold**, *italic*, `code`, links, and other formatting to the revised text. Code spans are protected with placeholders so the LLM cannot mangle them. This works reliably even with weaker models that strip Markdown
...
- Maximum selection: 4000 characters; timeout scales with text length (30–60s)
